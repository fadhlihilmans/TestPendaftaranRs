    ...
    <div class="relative bg-white rounded-xl shadow-xl w-full max-w-lg mx-4">
        ...
        <div class="p-6 md:p-8">
            <div class="flex items-center justify-between mb-6">
                <h4 class="text-xl font-semibold">{{ $formAdd ? 'Tambah' : 'Edit' }} Data Pendaftaran</h4>
            </div>
            
            <form wire:submit.prevent="{{ $formAdd ? 'add' : 'update' }}" class="space-y-5">
                <div>
                    <label class="block text-sm font-medium mb-1">Pasien</label>
                    <select wire:model="pasien_id" class="w-full px-3 py-2 border border-gray-200 rounded-lg focus:outline-none focus:ring-1 focus:ring-gray-900 focus:border-gray-200 @error('pasien_id') invalid @enderror">
                         <option value="">-- Pilih --</option>
                        @foreach($listPasien as $list)
                            <option value="{{ $list->id }}">{{ $list->nama_pasien }}</option>
                        @endforeach
                    </select>
                    @error('pasien_id') 
                ...
                <div>
                    ...
                    <select wire:model="poli_id" class="w-full px-3 py-2 border border-gray-200 rounded-lg focus:outline-none focus:ring-1 focus:ring-gray-900 focus:border-gray-200 @error('poli_id') invalid @enderror">
                         <option value="">-- Pilih --</option>
                        @foreach($listPoli as $list)
                            <option value="{{ $list->id }}">{{ $list->nama_poli }}</option>
                        @endforeach
                    </select>
                    @error('poli_id') 
                        <span class="text-red-500 text-xs mt-1">{{ $message }}</span> 
                ...
                <div>
                    ...
                    <select wire:model="dokter_id" class="w-full px-3 py-2 border border-gray-200 rounded-lg focus:outline-none focus:ring-1 focus:ring-gray-900 focus:border-gray-200 @error('dokter_id') invalid @enderror">
                         <option value="">-- Pilih --</option>
                        @foreach($listDokter as $list)
                            <option value="{{ $list->id }}">{{ $list->nama_dokter }}</option>
                        @endforeach
                    </select>
                    @error('dokter_id') 
                        <span class="text-red-500 text-xs mt-1">{{ $message }}</span> 
...
<div class="bg-white border border-gray-200 rounded-lg">
    ...
    <div class="p-6">
        {{-- <div class="mb-4 flex flex-col sm:flex-row sm:items-center justify-between space-y-2 sm:space-y-0">
            <div class="relative w-full sm:w-64">
                <div class="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
                    <i class="bx bx-search w-4 h-4"></i>
                </div>
                <input wire:model.live.debounce.500ms="search" type="text" class="block w-full pl-10 pr-3 py-2 rounded-xl border border-gray-200 bg-white text-sm focus:outline-none focus:ring-2 focus:ring-gray-900/20 focus:border-gray-300" placeholder="Cari...">
                @if($search)
                    <span 
                        wire:click="clearSearch" 
                        class="absolute inset-y-0 right-0 pr-3 flex items-center cursor-pointer text-gray-600 hover:text-gray-800">
                        <i class="bx bx-x"></i>
                    </span>
                @endif
            </div>
            <div class="flex items-center space-x-2">
                ...
                <select wire:model.live.debounce.500ms="perPage" class="text-sm border border-gray-200 bg-white rounded-lg px-2 py-1 focus:outline-none focus:ring-1 focus:ring-gray-900 focus:border-gray-200">
            ...
            <table class="min-w-full divide-y divide-light">
                <thead class="">
                    <tr>
                        ...
                        <th scope="col" class="px-6 py-3 text-left text-xs font-medium uppercase tracking-wider">No RM</th>
                        <th scope="col" class="px-6 py-3 text-left text-xs font-medium uppercase tracking-wider">Nama Pasien</th>
                        <th scope="col" class="px-6 py-3 text-left text-xs font-medium uppercase tracking-wider">Nama Poli</th>
                        <th scope="col" class="px-6 py-3 text-left text-xs font-medium uppercase tracking-wider">Nama Dokter</th>
                ...
                <tbody class="bg-white divide-y divide-light">
                    @forelse ($listPendaftaran as $item)
                        <tr class="divide-light">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="text-sm">{{ $item->tanggal_daftar ?? '-'}}</div>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="text-sm">{{ $item->pasien->nomor_rm ?? '-' }}</div>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="text-sm">{{ $item->pasien->nama_pasien ?? '-' }}</div>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="text-sm">{{ $item->poli->nama_poli ?? '-' }}</div>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap">
