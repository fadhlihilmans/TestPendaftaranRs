    ...
    <div class="relative bg-white rounded-xl shadow-xl w-full max-w-lg mx-4">
        ...
        <div class="p-6 md:p-8">
            <div class="flex items-center justify-between mb-6">
                <h4 class="text-xl font-semibold">{{ $formAdd ? 'Tambah' : 'Edit' }} Data Dokter</h4>
            </div>
            
            <form wire:submit.prevent="{{ $formAdd ? 'add' : 'update' }}" class="space-y-5">
                <div>
                    <label class="block text-sm font-medium mb-1">Nama Dokter</label>
                    <input wire:model="nama_dokter" type="text" class="w-full px-3 py-2  rounded-xl border border-gray-200 bg-white text-sm focus:outline-none focus:ring-2 focus:ring-gray-900/20 focus:border-gray-300 @error('nama_dokter') invalid @enderror">
                    @error('nama_dokter') 
        ...
        <div class="overflow-x-auto">
            <table class="min-w-full divide-y divide-light">
                ...
                <tbody class="bg-white divide-y divide-light">
                    @forelse ($listDokter as $item)
                        <tr class="divide-light">
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="text-sm">{{ $loop->iteration }}</div>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="text-sm">{{ $item->nama_dokter }}</div>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap">
