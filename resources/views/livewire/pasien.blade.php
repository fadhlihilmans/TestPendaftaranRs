    ...
    <div class="relative bg-white rounded-xl shadow-xl w-full max-w-lg mx-4">
        ...
        <div class="p-6 md:p-8">
            <div class="flex items-center justify-between mb-6">
                <h4 class="text-xl font-semibold">{{ $formAdd ? 'Tambah' : 'Edit' }} Data Pasien</h4>
            </div>
            
            <form wire:submit.prevent="{{ $formAdd ? 'add' : 'update' }}" class="space-y-5">
                <div>
                    <label class="block text-sm font-medium mb-1">Nomor RM</label>
                    <input wire:model="nomor_rm" type="text" class="w-full px-3 py-2  rounded-xl border border-gray-200 bg-white text-sm focus:outline-none focus:ring-2 focus:ring-gray-900/20 focus:border-gray-300 @error('nomor_rm') invalid @enderror">
                    @error('nomor_rm') 
                ...
                <div>
                    ...
                    <input wire:model="nama_pasien" type="text" class="w-full px-3 py-2  rounded-xl border border-gray-200 bg-white text-sm focus:outline-none focus:ring-2 focus:ring-gray-900/20 focus:border-gray-300 @error('nama_pasien') invalid @enderror">
                    @error('nama_pasien') 
                        <span class="text-red-500 text-xs mt-1">{{ $message }}</span> 
                    @enderror
                </div>
                ...
                <div>
                    ...
                    <textarea wire:model="alamat" 
                              rows="3" 
                              class="w-full px-3 py-2  rounded-xl border border-gray-200 bg-white text-sm focus:outline-none focus:ring-2 focus:ring-gray-900/20 focus:border-gray-300"></textarea>
                </div>
            ...
            <table class="min-w-full divide-y divide-light">
                <thead class="">
                    <tr>
                        ...
                        <th scope="col" class="px-6 py-3 text-left text-xs font-medium uppercase tracking-wider">Nomor RM</th>
                        <th scope="col" class="px-6 py-3 text-left text-xs font-medium uppercase tracking-wider">Nama Pasien</th>
                        <th scope="col" class="px-6 py-3 text-left text-xs font-medium uppercase tracking-wider">Alamat</th>
                ...
                <tbody class="bg-white divide-y divide-light">
                    @forelse ($listPasien as $item)
                        <tr class="divide-light">
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="text-sm">{{ $loop->iteration }}</div>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="text-sm">{{ $item->nomor_rm }}</div>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="text-sm">{{ $item->nama_pasien }}</div>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap">
